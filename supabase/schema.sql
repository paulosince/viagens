...
create policy "change log read own" on public.change_log for select using (user_id = auth.uid());
drop policy if exists "change log insert own" on public.change_log;
create policy "change log insert own" on public.change_log for insert with check (user_id = auth.uid());
