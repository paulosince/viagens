...
function dayEditorView(){
  ...
  return '<div class="edit-sheet-backdrop"><section class="edit-sheet" role="dialog" aria-modal="true" aria-label="Editar dia" data-stop-click="true"><header class="sheet-header"><button type="button" class="sheet-close sheet-cancel" data-action="cancel-edit-day" aria-label="Cancelar"><span class="close-icon" aria-hidden="true"></span></button><h2>'+esc(editorDate)+'</h2><button type="button" data-action="save-day-draft" class="sheet-done" aria-label="Confirmar">✓</button></header></section></div>';
}
