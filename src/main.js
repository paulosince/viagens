...
function activityEditor(){
  ...
  return '<form id="activity-edit-form" class="activity-editor card" data-form="activity" novalidate><div class="section-row"><div><span class="eyebrow">AGENDA DO DIA</span><h3>'+esc(a.title||'Nova entrada')+'</h3></div></div><div class="form-grid"><div class="field"><label>Horário</label><input name="time" data-draft-activity type="time" value="'+esc(time)+'"></div><div class="field"><label>Período</label><select name="period" data-draft-activity><option value="morning" '+(a.period==='morning'?'selected':'')+'>Manhã</option><option value="afternoon" '+(a.period==='afternoon'?'selected':'')+'>Tarde</option><option value="night" '+(a.period==='night'?'selected':'')+'>Noite</option></select></div><div class="field full"><label>Texto principal</label><input name="title" data-draft-activity value="'+esc(a.title||'')+'" placeholder="Ex.: Visitar a Torre Eiffel"></div><div class="field full"><label>Locais</label><input name="place_name" data-draft-activity value="'+esc(a.place_name||'')+'" placeholder="Selecione tags de locais"></div><div class="field full"><label>Detalhes</label><textarea name="description" data-draft-activity rows="3" placeholder="O que faremos e observações...">'+esc(a.description||'')+'</textarea></div><div class="field full"><label>Mais detalhes</label><textarea name="shopping_items" data-draft-activity rows="2" placeholder="Itens a comprar, um por linha">'+esc(a.shopping_items||'')+'</textarea><input name="meal" data-draft-activity value="'+esc(a.meal||'')+'" placeholder="Alimentação"><input name="transport" data-draft-activity value="'+esc(a.transport||'')+'" placeholder="Transporte"><textarea name="notes" data-draft-activity rows="2" placeholder="Notas">'+esc(a.notes||'')+'</textarea></div></div></form>';
}
